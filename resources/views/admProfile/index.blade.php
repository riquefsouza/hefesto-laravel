...
<form id="formListAdmProfile" style="padding: 5px;" action="/admProfile" method="get">
    ...
    <div class="card">
        <div class="card-header" style="font-weight: bold;font-size: large;">
            <span id="formTitle">{{ $messages["listAdmProfile.title"] }}</span>
        </div>
        <div class="card-body">
            <div class="row">
...
<form method="post">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="btnDelete" class="btn btn-primary" onclick="listAdmProfile.btnDeleteClick(this.form)">Yes</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <table class="table table-striped table-bordered" id="tableAdmProfile" style="width: 100%">
        <thead>
            <tr>
        ...
        <tbody>
            @foreach ($model as $item)
                <tr id="{{ $item->getIdAttribute() }}" onclick="listAdmProfile.tableRowClick(this);">
                    <td style="display: none">{{ $item->getIdAttribute() }}</td>
                    <td>{{ $item->getDescriptionAttribute() }}</td>
                    <td>{{ $item->getProfileUsersAttribute() }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
...
<script src="/static/js/admin/admProfile/listAdmProfile.js"></script>
